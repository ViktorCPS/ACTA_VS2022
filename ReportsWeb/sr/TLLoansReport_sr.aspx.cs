...
{
    public partial class TLLoansReport_sr : System.Web.UI.Page
    {
        const string pageName = "TLLoansReport_sr";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    DataSet ds = (DataSet)Session["TLLoansPage.dataSet"];
                    string employee = Session["TLLoansPage.employee"].ToString();
                    string tempUte = Session["TLLoansPage.tempUte"].ToString();
                    string fromDate = Session["TLLoansPage.fromDate"].ToString();
                    string toDate = Session["TLLoansPage.toDate"].ToString();
                    ReportParameter param1 = new ReportParameter("employee", employee);
                    ReportParameter param2 = new ReportParameter("tempUte", tempUte);
            ...
            catch (Exception ex)
            {
                try
                {
                    Response.Redirect("/ACTAWeb/CommonWeb/MessagesPages/ErrorPage.aspx?Error=Error in TLLoansReport_sr.Page_Load(): " + ex.Message.Replace('\n', ' ').Trim() + "&Header=" + Constants.falseValue.Trim(), false);
                }
