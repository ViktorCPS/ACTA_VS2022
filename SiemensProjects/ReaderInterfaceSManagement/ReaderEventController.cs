...
namespace ReaderInterfaceSManagement
{
	...
	public class ReaderEventController
	{
		...
		public void SetData(ArrayList employeeList, ArrayList tagList, 
			ArrayList readersList, ArrayList locationsList)
		{
			try
			{
			...
			catch(Exception ex)
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".StartMonitor() : " + ex.Message + "\n\n");  
				throw ex;
			}
		}
		...
		public int StopMonitor()
		{
			...
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".StopMonitor() : " + ex.Message);  
				throw ex;
			}
		...
		public int SetReaders()
		{
			try
			{
                //for(int j=0; j<readers.Count; j++)
                //{
                //    readerThreads.Add(new MonitorThread((Reader)readers[j], j+1));	
                //}
			}
			catch(Exception ex)
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".SetReaders() : " + ex.Message);  
				throw ex;
			}
		...
		public bool isActive(int readerID)
		{
			...
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".isActive() : " + ex.Message);  
				throw ex;
			}
		...
		public void SendMonitorNotification(Reader currentReader, int currentAntennaNum, UInt32 currentTagID)
		{
			try
			{
			...
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".NotifyAll() : " + ex.Message);  
				throw ex;
			}
		}
        ...
        public void RestartReading(DownloadManager mgrInstance)
		{
			...
			{
				debug.writeLog(DateTime.Now + " Exception in: " + 
					this.ToString() + ".ReastartReading() : " + ex.Message);  
				throw ex;
			}
		...
		public void ReaderdAction(Reader currentReader, bool isStarted, bool isSucceeded, 
		...
		public void ReaderDataExchange(Reader reader, bool isStarted, DateTime nextReading, string actionName)
		{
			try
			{
                //NotificationController notifController = NotificationController.GetInstance();
		...
		public void ReaderFailed(Reader reader, bool isNetExist, bool isDataTranExist)
		{
			try
			{
                //NotificationController notifController = NotificationController.GetInstance();
