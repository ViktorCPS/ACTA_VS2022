...
{
    [ServiceContract(Namespace = "http://ReaderControlService")]
    ...
    public class ReaderControlService : IReaderControlService
    {
        private static ReaderControlService instance = null;
        ...
        private ReaderControlService()
        {
            // uncomment to enable pause for attaching process to debugger
            //System.Threading.Thread.Sleep(30000);

            // Init DebugLog
            NotificationController.SetApplicationName("ACTAReaderControlService");
        ...
        public static ReaderControlService Instance
        {
            get
            {
                lock (instanceLocker)
                {
                    if (instance == null)
                    {
                        instance = new ReaderControlService();
                    }
                    return instance;
                }
            }
        ...
        private void PingAlert(Object sender, NotificationEventArgs args)
        {
            try
            {
                lock (locker)
                {
                    ...
                    foreach (ReaderTO reader in readers)
                    {
                        if (reader.ReaderID == args.reader.ReaderID)
                        {
                            ...
                            readersStatuses.AlertReaderID = readerID;
                            readersStatuses.StatusesData[readerID].IsNetExist = args.isNetExist;
                            readersStatuses.StatusesData[readerID].IsPingAlertDone = true;
            ...
            {
                log.writeLog(DateTime.Now + " Exception in: " + this.ToString() + ".PingAlert(): " + ex.Message + "\n");
            }
        }
