...
{
	/// <summary>
	/// Summary description for MySQLApplUserDAO.
	/// </summary>
	public class MySQLApplUserDAO : ApplUserDAO
	{
        ...
        public MySQLApplUserDAO(MySqlConnection mySqlConnection)
        {
		...
		public int insert(ApplUserTO userTO)
		{
			...
			try
			{
                ...
                {
                    sbInsert.Append("'" + userTO.ExtraHoursAdvancedAmt + "' )");
                }
		...
		public bool updateExitPermVerification(ApplUserTO userTO)
		{
			...
			try
			{
                ...
                if (userTO.ExitPermVerification != -1)
                {
                    sbUpdate.Append("extra_hrs_advanced_amt = '" + userTO.ExtraHoursAdvancedAmt.ToString().Trim() + "'");
                }
                else
                {
		...
		public ApplUserTO find(string userID)
		{
			...
			try
			{
				MySqlCommand cmd = new MySqlCommand( "SELECT * FROM appl_users WHERE user_id = N'" + userID.Trim() + "'", conn );
				...
				if (table.Rows.Count == 1)
				{
                    ...
                    {
                        applUserTO.ExtraHoursAdvancedAmt = int.Parse(table.Rows[0]["extra_hrs_advanced_amt"].ToString().Trim());
                    }
				}
			}
			catch(Exception ex)
