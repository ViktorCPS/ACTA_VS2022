...
{
	/// <summary>
	/// Summary description for MySQLReaderDAO.
	/// </summary>
	public class MySQLReaderDAO : ReaderDAO
	{
        ...
        public MySQLReaderDAO(MySqlConnection mySqlConnection)
        {
		...
		public int insert(ReaderTO readerData)
		{
			...
			catch(MySqlException ex)
			{
				...
				{
					throw new Exception("Reader with given ID already exists.");
				}
				else
				{
        ...
        public List<ReaderTO> searchForIDs(string readerID)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("SELECT * FROM readers WHERE reader_id IN (" + readerID.Trim() + ")", conn);
		...
		public ReaderTO find(string readerID)
		{
			...
			try
			{
				MySqlCommand cmd = new MySqlCommand( "SELECT * FROM readers WHERE reader_id = '" + readerID.Trim() + "'", conn );
				MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
		...
		public int findMAXReaderID()
		{
			...
			try
			{
				MySqlCommand cmd = new MySqlCommand( "SELECT MAX(reader_id) AS reader_id FROM readers", conn );
				MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
            ...
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        {
                            readTO.DownloadStartTime = DateTime.Parse(row["download_start_time"].ToString());
                        }
			...
			try
			{
				...
				if (table.Rows.Count > 0)
				{
					foreach(DataRow row in table.Rows)
					{
						...
						if (row["reader_id"] != DBNull.Value)
						{
							readTO.ReaderID = Int32.Parse(row["reader_id"].ToString().Trim());
						}
						if (row["description"] != DBNull.Value)
						{
							readTO.Description = row["description"].ToString().Trim();
						...
						{
							readTO.LastReadTime = DateTime.Parse(row["readerTime"].ToString().Trim());
						}
		...
		public DateTime getAllReadersLastReadTime()
		{
			...
			try
			{
				...
				if (table.Rows.Count == 1)
				{
					...
					{
						allReadersTime = DateTime.Parse(table.Rows[0]["allReadersTime"].ToString().Trim());
					}
				}
			}
			catch(Exception ex)
