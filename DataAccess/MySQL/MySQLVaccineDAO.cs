...
{
    public class MySQLVaccineDAO : VaccineDAO
    {
        ...
        public MySQLVaccineDAO(MySqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DAOController.GetInstance();
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
        ...
        public List<VaccineTO> getVaccines(VaccineTO vacTO)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand(select + " ORDER BY vaccine_type", conn);
