...
{
    public class MySQLMedicalCheckVisitHdrDAO : MedicalCheckVisitHdrDAO
    {
        ...
        public MySQLMedicalCheckVisitHdrDAO()
        {
        ...
        public MySQLMedicalCheckVisitHdrDAO(MySqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DAOController.GetInstance();
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
        ...
        public List<MedicalCheckVisitHdrTO> getEmptyVisits(string visitIDs)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd;
                if (SqlTrans != null)
                    cmd = new MySqlCommand(select + " ORDER BY employee_id, schedule_date", conn, SqlTrans);
                else
                    cmd = new MySqlCommand(select + " ORDER BY employee_id, schedule_date", conn);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
        ...
        public List<MedicalCheckVisitHdrTO> getMedicalCheckVisitHeaders(MedicalCheckVisitHdrTO hdrTO)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand(select + " ORDER BY employee_id, schedule_date", conn);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
        ...
        public Dictionary<uint, MedicalCheckVisitHdrTO> getMedicalCheckVisits(string emplIDs, string status, string point, string check, string type, DateTime from, DateTime to)
        {
            ...
            try
            {
                ...
                MySqlCommand cmdDtl = new MySqlCommand(selectDtl, conn);
                MySqlDataAdapter sqlDataAdapterDtl = new MySqlDataAdapter(cmdDtl);
                ...
                MySqlCommand cmd = new MySqlCommand("SELECT * " + select + " ORDER BY employee_id, schedule_date", conn);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
        ...
        public List<MedicalCheckVisitHdrTO> getMedicalCheckVisits(string visidIDs)
        {
            DataSet dataSet = new DataSet();
            DataSet dataSetDtl = new DataSet();
            MedicalCheckVisitHdrTO hdr = new MedicalCheckVisitHdrTO();
            ...
            try
            {
                if (visidIDs.Trim().Length <= 0)
                    return hdrList;

                select = "SELECT * FROM medical_chk_visits_hdr WHERE visit_id IN (" + visidIDs.Trim() + ") ORDER BY employee_id, schedule_date";

                // get visit details
                string selectDtl = "SELECT * FROM medical_chk_visits_dtl WHERE visit_id IN (" + visidIDs.Trim() + ")";
                MySqlCommand cmdDtl = new MySqlCommand(selectDtl, conn);
                MySqlDataAdapter sqlDataAdapterDtl = new MySqlDataAdapter(cmdDtl);
