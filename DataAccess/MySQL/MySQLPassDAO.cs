...
{
	/// <summary>
	/// Summary description for MySQLPassDAO.
	/// </summary>
	public class MySQLPassDAO : PassDAO
	{
        ...
        public MySQLPassDAO(MySqlConnection mySqlConnection)
        {
			...
			int locationID, int manualCreated, int isWrkHrsCount)
		{
			...
			catch(MySqlException sqlex)
			{
				...
				{
					throw new Exception("Pass with given ID already exists.");
				}
				else
				{
		...
		{			
			...
			try
			{
				...
				cmd = new MySqlCommand( sbInsert.ToString(), conn, sqlTrans );
				rowsAffected = cmd.ExecuteNonQuery();
				if (doCommit)
				{
					sqlTrans.Commit();
        ...
        public int insertGetID(PassTO passTO, bool doCommit)
        {
            ...
            try
            {
                ...
                sbInsert.Append(" N'" + DAOController.GetLogInUser().Trim() + "', NOW()); ");
                sbInsert.Append("SELECT LAST_INSERT_ID() AS pass_id ");
        ...
        public int insert(List<PassTO> passTOList, ExitPermissionTO perm, string createdBy, DAOFactory factory)
		{
			...
			MySQLExitPermissionDAO exitPermDAO = (MySQLExitPermissionDAO) factory.getExitPermissionsDAO(conn);
		...
		public bool delete(string passID)
		{
			...
			try
			{
                ...
                sbSelect.Append("SELECT employee_id, date_format(event_time, '%Y-%m-%d') currentDate ");
                sbSelect.Append("FROM passes ");
                sbSelect.Append("WHERE ");
                sbSelect.Append("pass_id = " + Int32.Parse(passID.Trim()));

                MySqlCommand cmdS = new MySqlCommand(sbSelect.ToString(), conn, sqlTrans);
                MySqlDataAdapter sqlDA = new MySqlDataAdapter(cmdS);
                ...
                if (isDeleted)
                {
                    ...
                    sbSelect.Append(" AND convert(date_format(event_time, '%Y-%m-%d'), datetime) = convert('" + currentDate + "', datetime)");

                    cmdS.CommandText = sbSelect.ToString();
                    MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmdS);
                    DataSet dataSet = new DataSet();
                    int count = 0;
                    ...
                    if (count > 0)
                    {
                        ...
                        sbUpdate.Append(" AND convert(date_format(event_time, '%Y-%m-%d'), datetime) = convert('" + currentDate + "', datetime)");

                        MySqlCommand cmdU = new MySqlCommand(sbUpdate.ToString(), conn, sqlTrans);
		...
		public PassTO find(string passID)
		{
			...
			try
			{
				...
				if (table.Rows.Count == 1)
				{
					...
					{
						pass.EventTime = DateTime.Parse(table.Rows[0]["event_time"].ToString());
					}
		...
		public bool update(int employeeID, string date, bool doCommit)
		{
			...
			try
			{
				...
				sbUpdate.Append("AND DATE_FORMAT(event_time,GET_FORMAT(DATE,'ISO')) = '" + DateTime.Parse(date).ToString("yyyy-MM-dd").Trim() + "'");
        ...
        {            
            ...
            try
            {
                ...
                sbUpdate.Append("modified_time = NOW() ");
                sbUpdate.Append("WHERE  ");
                sbUpdate.Append(" DATE_FORMAT(event_time,GET_FORMAT(DATE,'ISO')) >= '" + startDate.ToString("yyyy-MM-dd").Trim() + "'");
                sbUpdate.Append("AND DATE_FORMAT(event_time,GET_FORMAT(DATE,'ISO')) <= '" + endDate.ToString("yyyy-MM-dd").Trim() + "'");
			...
			try
			{
                ...
                if (isUpdated && !passTO.EventTime.Date.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture).Equals(currentDate))
                {
                    ...
                    if (count > 0)
                    {
                        ...
                        sbUpdate1.Append("modified_time = NOW() ");
                        sbUpdate1.Append("WHERE ");
                        sbUpdate1.Append("employee_id = " + employeeID);
                        sbUpdate1.Append(" AND convert(date_format(event_time, '%Y-%m-%d'), datetime) = convert('" + currentDate + "', datetime)");

                        MySqlCommand cmdU = new MySqlCommand(sbUpdate1.ToString(), conn, sqlTrans);
                        int resU = cmdU.ExecuteNonQuery();
                        if (resU <= 0)
                        {
                            isUpdated = false;
        ...
        public List<PassTO> getPassForPerm(int employeeID, string direction, DateTime eventTime, int isWrkHrsCount)
        {
            ...
            try
            {
                ...
                if ( (employeeID != -1) || (!direction.Trim().Equals("")) ||
                    (!eventTime.Equals(new DateTime())) || (isWrkHrsCount != -1))
                {
                    sb.Append(" WHERE ");
                    ...
                    if (!direction.Trim().Equals(""))
                    {
                        sb.Append(" UPERR(direction) = '" + direction.Trim().ToUpper() + "' AND");
                    }
                    if (!eventTime.Equals(new DateTime(0)))
                    {
                        sb.Append(" event_time = '" + eventTime.ToString(dateTimeformat).Trim().ToUpper() + "' AND");
                    }                    
                    if (isWrkHrsCount != -1)
                    {
                        //sb.Append(" UPPER(is_wrk_hrs) LIKE '" + isWrkHrsCount.ToString().Trim().ToUpper() + "' AND");
                        sb.Append(" is_wrk_hrs = '" + isWrkHrsCount.ToString().Trim() + "' AND");
                    }
			...
			try
			{
                    ...
                    || (!advFromTime.TimeOfDay.Equals(fromSpan) || !advToTime.TimeOfDay.Equals(toSpan)))
				{
                    if (passTO.EmployeeID != -1)
					{
                        ...
                        sb.Append(" ps.employee_id = '" + passTO.EmployeeID.ToString().Trim().ToUpper() + "' AND");
					}
                    if (!passTO.Direction.Trim().Equals(""))
					{
                        sb.Append(" RTRIM(LTRIM(UPPER(ps.direction))) LIKE N'%" + passTO.Direction.Trim().ToUpper() + "%' AND");
					}
					if (!fromTime.Equals(new DateTime()) && !toTime.Equals(new DateTime(0)))
					{
                        ...
                        sb.Append(" ps.event_time >= CONVERT('" + fromTime.ToString("yyyy-MM-dd") + "', datetime) AND ");
                        sb.Append(" ps.event_time < CONVERT('" + toTime.AddDays(1).ToString("yyyy-MM-dd") + "', datetime) AND ");
					}
                    if (passTO.PassTypeID != -1)
					{
                    ...
                    if (!advFromTime.TimeOfDay.Equals(fromSpan) || !advToTime.TimeOfDay.Equals(toSpan))
                    {
                        if (!advFromTime.TimeOfDay.Equals(fromSpan))
                            sb.Append(" CONVERT(CONCAT('1900-01-01T',DATE_FORMAT(ps.event_time,'%H:%i:%s')), datetime) >= CONVERT('1900-01-01T" + advFromTime.ToString("HH:mm:ss") + "', datetime) AND ");
                        if (!advToTime.TimeOfDay.Equals(toSpan))
                            sb.Append(" CONVERT(CONCAT('1900-01-01T',DATE_FORMAT(ps.event_time,'%H:%i:%s')), datetime) < CONVERT('1900-01-01T" + advToTime.AddMinutes(1).ToString("HH:mm:ss") + "', datetime) AND ");
                    }
		...
		public int getPassesIntervalCount(PassTO passTO, DateTime fromTime, DateTime toTime, string wUnits, DateTime advFromTime, DateTime advToTime)
		{
			...
			try
			{
				...
				select += "ps.employee_id = empl.employee_id AND empl.working_unit_id IN (" + wUnits + ");";
		...
		public List<PassTO> getPassesList(PassTO passTO)
		{
			...
			try
			{
                ...
                if ((passTO.PassID != -1) ||
                    ...
                    (passTO.IsWrkHrsCount != -1) || (passTO.PairGenUsed != -1))
				{
                    if (passTO.PassID != -1)
					{
                    ...
                    if (!passTO.EventTime.Equals(new DateTime()))
					{
                        sb.Append(" UPPER(DATE_FORMAT(ps.event_time,GET_FORMAT(DATETIME,'ISO'))) LIKE '" + passTO.EventTime.ToString(dateTimeformat) + "' AND");
					}
                    if (passTO.PassTypeID != -1)
					{
                    ...
                    if (passTO.IsWrkHrsCount != -1)
					{
                        ...
                        sb.Append(" ps.is_wrk_hrs = '" + passTO.IsWrkHrsCount.ToString().Trim() + "' AND");
					}
        ...
        public void logToPass(List<PassTO> passTOList, List<LogTO> logTOList, DAOFactory factory)
		{
			...
			try
			{
				MySqlCommand cmdLog = new MySqlCommand( "SELECT * FROM log", conn);
				MySqlDataAdapter sqlDataAdapterLog = new MySqlDataAdapter(cmdLog);

				MySqlCommandBuilder builderLog = new MySqlCommandBuilder(sqlDataAdapterLog);

				sqlDataAdapterLog.MissingSchemaAction = MissingSchemaAction.AddWithKey;
				sqlDataAdapterLog.Fill(dataSet, "log");

				MySqlCommand cmdPass = new MySqlCommand( "SELECT * FROM passes", conn);
				MySqlDataAdapter sqlDataAdapterPass = new MySqlDataAdapter(cmdPass);

				MySqlCommandBuilder bulderPass = new MySqlCommandBuilder(sqlDataAdapterPass);
		...
		{
			MySQLLogDAO logDAO = (MySQLLogDAO) factory.getLogDAO(conn);
			MySQLExitPermissionDAO exitPermDAO = (MySQLExitPermissionDAO) factory.getExitPermissionsDAO(conn);
        ...
        public List<PassTO> getDiferencePasses(DateTime lastReadingTime)
        {
            ...
            try
            {
                ...
                sb.Append("AND ");
                sb.Append("p.created_time > CONVERT('" + lastReadingTime.ToString("yyyy-MM-dd HH:mm:ss") + "', datetime) AND ");
                ...
                sb.Append("AND ");
                sb.Append("p.modified_time >  CONVERT('" + lastReadingTime.ToString("yyyy-MM-dd HH:mm:ss") + "', datetime) AND ");
                sb.Append(") as gry ");
		...
		public ArrayList getEmpoloyeesByDate()
		{
			...
			try
			{
				...
				sb.Append("SELECT DISTINCT employee_id, DATE_FORMAT(event_time,'%Y-%m-%d') AS workday "
		...
		public List<PassTO> getPassesForEmployee(int empoyeeID, string date)
		{
			...
			{
				// CHANGED: Take only unused passes
				MySqlCommand cmd = new MySqlCommand("select * from passes where employee_id = " + empoyeeID + " and " +
					" DATE_FORMAT(event_time,GET_FORMAT(DATE,'ISO')) = '" + DateTime.Parse(date).ToString("yyyy-MM-dd") + "' and pair_gen_used = '" + ((int) Constants.PairGenUsed.Unused) + "' order by event_time, pass_id ", conn);
        ...
        public Dictionary<int, Dictionary<DateTime, List<PassTO>>> getPassesForEmployeesPeriod(string employeesID, DateTime from, DateTime to)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand(select.Trim(), conn);
        ...
        public List<PassTO> getPassesForEmployeeAll(int empoyeeID, DateTime date)
        {
            ...
            try
            {
                // CHANGED: Take only unused passes
                MySqlCommand cmd = new MySqlCommand("select * from passes where employee_id = " + empoyeeID + " and " +
                    " DATE_FORMAT(event_time,GET_FORMAT(DATE,'ISO')) = '" + date.ToString("yyyy-MM-dd") + "' AND UPPER(direction) = 'IN' order by event_time, pass_id ", conn);
        ...
        public List<PassTO> getPassesForEmployeeSched(int employeeID)
        {
            ...
            {
                // CHANGED: Take only unused passes
                MySqlCommand cmd = new MySqlCommand("select * from passes where employee_id = " + employeeID + "' AND UPPER(direction) = 'IN' order by event_time, pass_id ", conn);
		...
		public List<PassTO> getPassesForExitPerm(int employeeID, DateTime eventTime)
		{
			...
			try
			{
				...
				sbSelect.Append("DATE_FORMAT(ps.event_time,'%m/%d/%Y') = '" + eventTime.ToString("MM/dd/yyy", ci) + "' AND ");
				...
				MySqlCommand cmd = new MySqlCommand(sbSelect.ToString(), conn );
		...
		public List<PassTO> getPassesForExitPerm(int employeeID, DateTime startTime, int offset)
		{
			...
			try
			{
				...
				sbSelect.Append("DATE_FORMAT(event_time,GET_FORMAT(DATETIME,'ISO')) >= '" + startTime.ToString("yyy-MM-dd HH:mm:ss") + "' AND ");
				sbSelect.Append("DATE_FORMAT(event_time,GET_FORMAT(DATETIME,'ISO')) <= '" + startTime.AddMinutes(offset).ToString("yyy-MM-dd HH:mm:ss") + "' AND ");
                sbSelect.Append(" ps.is_wrk_hrs = 1 AND ");
                sbSelect.Append(" ps.pass_type_id = 0 AND ");
		...
		public List<PassTO> getPermPassesForPair(int employeeID, DateTime startTime, DateTime endTime)
		{
			...
			try
			{
				...
				sbSelect.Append("AND ((DATE_FORMAT(event_time,'%Y-%m-%d %H:%i:%s') = '" + startTime.ToString("yyy-MM-dd HH:mm:ss") + "' ");
				sbSelect.Append("AND UPPER(direction) = N'" + Constants.DirectionIn.ToUpper() + "') ");
				sbSelect.Append("OR (DATE_FORMAT(event_time,'%Y-%m-%d %H:%i:%s') = '" + endTime.ToString("yyy-MM-dd HH:mm:ss") + "' ");
        ...
        public List<PassTO> getCurrentPasses(PassTO passTO, DateTime fromTime, DateTime toTime, string wUnits, string modifiedBy)
        {
            ...
            try
            {
                ...
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT ps.*, empl.last_name, empl.first_name, pt.description, loc.name ");
                sb.Append("FROM passes ps, employees empl, pass_types pt, locations loc, passes_hist ph ");
                sb.Append("WHERE ");

                if ((passTO.EmployeeID != -1) || (!passTO.Direction.Trim().Equals("")) ||
                    (!fromTime.Equals(new DateTime())) || (!toTime.Equals(new DateTime())) ||
                    (passTO.PassTypeID != -1) || (passTO.LocationID != -1)
                    || (!modifiedBy.Trim().Equals("")))
                {
                    if (passTO.EmployeeID != -1)
                    {
                        //sb.Append(" UPPER(ps.employee_id) LIKE '" + employeeID.ToString().Trim().ToUpper() + "' AND");
                        sb.Append(" ph.employee_id = '" + passTO.EmployeeID.ToString().Trim().ToUpper() + "' AND");
                    }
                    if (!passTO.Direction.Trim().Equals(""))
                    {
                    ...
                    {
                        //sb.Append(" UPPER(ps.pass_type_id) LIKE '" + passTypeID.ToString().Trim().ToUpper() + "' AND");
                        sb.Append(" ph.pass_type_id = '" + passTO.PassTypeID.ToString().Trim().ToUpper() + "' AND");
                    }
                    if (passTO.LocationID != -1)
                    {
                        //sb.Append(" UPPER(ps.location_id) LIKE '" + locationID.ToString().Trim().ToUpper() + "' AND");
                        sb.Append(" ph.location_id = '" + passTO.LocationID.ToString().Trim().ToUpper() + "' AND");
                    }

                    if (!modifiedBy.Equals("") && !modifiedBy.Equals("*"))
                    {
                        sb.Append(" ph.modified_by = N'" + modifiedBy.Trim() + "' AND");
                    }

                    //select = sb.ToString(0, sb.ToString().Length - 3);
                }

                sb.Append(" empl.employee_id = ps.employee_id AND pt.pass_type_id = ps.pass_type_id AND ps.pass_id = ph.pass_id AND ");
                sb.Append("loc.location_id = ps.location_id ");
                sb.Append("GROUP BY pass_id;");
		...
		public PassTO getPassBeforePermissionPass(int emplID, DateTime eventTime)
		{
			...
			{
				StringBuilder sbSelect = new StringBuilder();
				sbSelect.Append("SELECT * FROM passes WHERE employee_id = '" + emplID.ToString().Trim() + "' ");
				sbSelect.Append("AND is_wrk_hrs = '" + (int) Constants.IsWrkCount.IsCounter + "' ");
				sbSelect.Append("AND DATE_FORMAT(event_time,'%m/%d/%Y') = '" + eventTime.ToString("MM/dd/yyy") + "' ");
				sbSelect.Append("AND TIME_FORMAT(event_time,GET_FORMAT(TIME,'ISO')) < '" + eventTime.ToString("HH:mm:ss") + "' ");
				sbSelect.Append("ORDER BY event_time DESC ");
				sbSelect.Append("LIMIT 1");
				MySqlCommand cmd = new MySqlCommand(sbSelect.ToString(), conn );
				...
				if (table.Rows.Count > 0)
				{
                    ...
                    {
                        pass.CreatedTime = DateTime.Parse(table.Rows[0]["created_time"].ToString());
                    }
				}			
			}
			catch(Exception ex)
        ...
        public bool compareGates(int gateID, int organUnitID)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("select gate_id from organizational_unit_x_gate where organizational_unit_id= '" + organUnitID + "'", conn);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
                ...
                if (table.Rows.Count == 1)
                {
                    ...
                    if (table.Rows[0]["gate_id"] != DBNull.Value)
                    {
                        foundGate = Int32.Parse(table.Rows[0]["gate_id"].ToString().Trim());
                    }
                }
                if (foundGate == gateID)
                {
        ...
        public int getOrganUnitID(ulong tagID)
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand(" select e.* from employees e join tags t on e.employee_id=t.owner_id where t.tag_id= '" + tagID + "'", conn);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
