...
{
	/// <summary>
	/// Summary description for MySQLWorkTimeShemaDAO.
	/// </summary>
	public class MySQLWorkTimeShemaDAO : WorkTimeSchemaDAO
	{
		
		MySqlConnection conn = null;
		protected string dateTimeformat = "";
		
		public MySQLWorkTimeShemaDAO()
		{
        ...
        public MySQLWorkTimeShemaDAO(MySqlConnection mySqlConnection)
        {
		...
		public int insert(WorkTimeSchemaTO wtSchemaTO)
		{
			...
			try
			{
				...
				sbInsert.Append("SELECT LAST_INSERT_ID() AS schema_id ");
        ...
        // Same as method getWorkTimeSchemas(string timeSchemaID, string timeSchemaName, string decsription, string timeSchemaType, string duration, IDBransaction trans)
        ...
        public List<WorkTimeSchemaTO> getWorkTimeSchemas(WorkTimeSchemaTO tsTO)
		{
			...
			try
			{
				...
				if (tsTO.TimeSchemaID != -1)
				{
					sb.Append("hdr.time_schema_id = '" + tsTO.TimeSchemaID.ToString() + "' AND ");
				}
                if (!tsTO.Name.Trim().Equals(""))
				{
                    sb.Append("UPPER(hdr.name) LIKE N'%" + tsTO.Name.Trim().ToUpper() + "%' AND ");
        ...
        public List<WorkTimeSchemaTO> getWorkTimeSchemas(string timeSchemaID)
        {
            ...
            try
            {
                ...
                {
                    sb.Append("UPPER(hdr.time_schema_id) IN (" + timeSchemaID + ") AND ");
                }
        ...
        public List<WorkTimeSchemaTO> getWorkTimeSchemas(WorkTimeSchemaTO tsTO, IDbTransaction trans)
        {
            ...
            try
            {
                ...
                if (!tsTO.Type.Trim().Equals(""))
                {
                    sb.Append("UPPER(hdr.time_schema_type) LIKE N'%" + tsTO.Type.ToUpper() + "%' AND ");
                }
                if (tsTO.CycleDuration != -1)
                {
                    having = "having count(distinct dtl.cycle_day) = '" + tsTO.CycleDuration.ToString().Trim() + "' ";
        ...
        private Dictionary<int, Dictionary<int, WorkTimeIntervalTO>> getWorkTimeSchemaDetails(int timeSchemaID, IDbTransaction trans)
        {
            ...
            try
            {
                ...
                MySqlCommand cmd = new MySqlCommand(sb.ToString(), conn, (MySqlTransaction) trans);
                MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
		...
		private string prepareIntervalInsert(WorkTimeIntervalTO interval, string schema_id)
		{
			...
			sbInsert.Append("'" + interval.StartTime.ToString(dateTimeformat) + "', '" + interval.EndTime.ToString(dateTimeformat) + "', '" + interval.EarliestArrived.ToString(dateTimeformat) + "', ");
			sbInsert.Append("'" + interval.LatestArrivaed.ToString(dateTimeformat) + "', '" + interval.EarliestLeft.ToString(dateTimeformat) + "', '" + interval.LatestLeft.ToString(dateTimeformat) +"', " + interval.AutoClose.ToString().Trim() + ", ");
			if (interval.PauseID != -1)
				sbInsert.Append(interval.PauseID.ToString().Trim() + ", ");
			else
				sbInsert.Append("null, ");
            sbInsert.Append("N'" + interval.Description.Trim() + "', ");
			sbInsert.Append("N'" + DAOController.GetLogInUser().Trim() + "', NOW())");
		...
		private string prepareIntrvalUpdate(WorkTimeIntervalTO interval)
		{
			...
			sb.Append("start_time = '" + interval.StartTime.ToString(dateTimeformat) + "', ");
			sb.Append("end_time = '" + interval.EndTime.ToString(dateTimeformat) + "', ");
			sb.Append("ea_time = '" + interval.EarliestArrived.ToString(dateTimeformat) + "', ");
			sb.Append("la_time = '" + interval.LatestArrivaed.ToString(dateTimeformat) + "', ");
			sb.Append("el_time = '" + interval.EarliestLeft.ToString(dateTimeformat) + "', ");
			sb.Append("ll_time = '" + interval.LatestLeft.ToString(dateTimeformat) + "', ");
            ...
            sb.Append("description = N'" + interval.Description + "', ");
			sb.Append("modified_by = N'" + DAOController.GetLogInUser().Trim() + "', ");
			sb.Append("modified_time = NOW() ");
