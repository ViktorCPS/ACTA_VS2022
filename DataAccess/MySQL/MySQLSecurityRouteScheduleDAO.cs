...
{
    public class MySQLSecurityRouteScheduleDAO : SecurityRouteScheduleDAO
    {
        ...
        public MySQLSecurityRouteScheduleDAO()
		{
			conn = MySQLDAOFactory.getConnection();	
			DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
		}
        public MySQLSecurityRouteScheduleDAO(MySqlConnection mySqlConnection)
        {
        ...
        public ArrayList getRoutesSch(int emplID, int routeID, DateTime from, DateTime to)
        {
			...
			try
			{
				...
				if((emplID != -1) || (routeID != -1) || (!from.Equals(new DateTime(0))) || (!to.Equals(new DateTime(0))))
				{
					...
					{
                        sb.Append(" rs.date >= CONVERT('" + from.ToString("yyyy-MM-dd") + "', datetime) AND ");
                        sb.Append(" rs.date < CONVERT('" + to.AddDays(1).ToString("yyyy-MM-dd") + "', datetime) AND ");
					}
				}
        ...
        public bool delete(int emplID, int routeID, DateTime date)
        {
            ...
            try
            {
                ...
                select = "DELETE FROM security_routes_schedule WHERE employee_id = '" + emplID.ToString().Trim() + "' "
                    ...
                    + "AND DATE_FORMAT(date, '%Y-%m-%d') = '" + date.ToString("yyyy-MM-dd") + "'";
