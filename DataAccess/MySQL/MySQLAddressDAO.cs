...
{
	/// <summary>
	/// Summary description for MySQLAddressDAO.
	/// </summary>
	public class MySQLAddressDAO : AddressDAO
	{
        ...
        public MySQLAddressDAO(MySqlConnection mySqlConnection)
        {
			...
			string telNumber2, string telNumber3, string faxNumber1, string faxNumber2, string faxNumber3)
		{
			...
			try
			{
				...
				sbInsert.Append("SELECT LAST_INSERT_ID() AS address_id ");
		...
		{			
			...
			try
			{
				MySqlCommand cmd = new MySqlCommand( "SELECT * FROM addresses WHERE address_id = '" + addressID.ToString().Trim() + "'", conn );
				MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
