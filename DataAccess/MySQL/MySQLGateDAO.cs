...
{
	/// <summary>
	/// Summary description for MySQLGateDAO.
	/// </summary>
	public class MySQLGateDAO : GateDAO
	{
        ...
        public MySQLGateDAO(MySqlConnection mySqlConnection)
        {
		...
		public int insert(string name, string description, DateTime downloadStartTime, int downloadInterval, int downloadEraseCounter)
		{
			...
			try
			{
				...
				sbInsert.Append("SELECT LAST_INSERT_ID() AS gate_id ");
            ...
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        if (!row["download_start_time"].Equals(DBNull.Value))
                        {
                            gateTO.DownloadStartTime = DateTime.Parse(row["download_start_time"].ToString());
                        }
                        if (!row["download_interval"].Equals(DBNull.Value))
                        {
                            gateTO.DownloadInterval = Int32.Parse(row["download_interval"].ToString().Trim());
		...
		public GateTO find(int gateID)
		{
			...
			try
			{
				MySqlCommand cmd = new MySqlCommand( "SELECT * FROM gates WHERE gate_id = '" + gateID.ToString().Trim() + "'", conn );
				MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
				...
				if (table.Rows.Count == 1)
				{
					...
					{
						gateTO.DownloadStartTime = DateTime.Parse( table.Rows[0]["download_start_time"].ToString());
					}
        ...
        public List<GateTO> getGates(GateTO gTO)
		{
			...
			try
			{
				...
				if((gTO.GateID != -1) || (!gTO.Name.Trim().Equals("")) || 
					...
					(gTO.DownloadInterval != -1) || (gTO.DownloadEraseCounter != -1))
				{
                    sb.Append(" AND");
					...
					{
						sb.Append(" DATE_FORMAT(download_start_time,GET_FORMAT(TIME,'ISO') = '" + gTO.DownloadStartTime.ToString(dateTimeformat).Trim() + "' AND");
					}
		...
		public GateTO findGetAccessProfile(string gateID)
		{
			...
			try
			{
				//MySqlCommand cmd = new MySqlCommand( "SELECT gate_id, name, description, gate_timeaccess_profile_id FROM gates WHERE gate_id = '" + gateID.Trim() + "'" , conn );
                MySqlCommand cmd;
                if (this.SqlTrans == null)
                    cmd = new MySqlCommand("SELECT gate_id, name, description, gate_timeaccess_profile_id FROM gates WHERE gate_id = '" + gateID.Trim() + "'", conn);
                else
                    cmd = new MySqlCommand("SELECT gate_id, name, description, gate_timeaccess_profile_id FROM gates WHERE gate_id = '" + gateID.Trim() + "'", conn, this.SqlTrans);
				MySqlDataAdapter sqlDataAdapter = new MySqlDataAdapter(cmd);
