...
{
    public class MySQLSyncResponsibilityHistDAO:SyncResponsibilityHistDAO
    {
		...
		public MySQLSyncResponsibilityHistDAO()
		{
        ...
        public MySQLSyncResponsibilityHistDAO(MySqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
            dateTimeformat = dateTimeFormat.SortableDateTimePattern;
        ...
        public bool insert(SyncResponsibilityTO syncRes, bool doCommit)
        {
            if (doCommit)
            {
            ...
            try
            {
                ...
                sbInsert.Append("(rec_id, unit_id, responsible_persone_id,  structure_type, ");
                sbInsert.Append("valid_from, valid_to, result, remark, created_by, created_time, created_time_hist) ");
                sbInsert.Append("VALUES (");
                sbInsert.Append(" " + syncRes.RecID + ",");
                sbInsert.Append(" " + syncRes.UnitID + ",");
                if (syncRes.ResponsiblePersonID != -1)
                {
                    sbInsert.Append(" '" + syncRes.ResponsiblePersonID + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                }
                if (syncRes.StructureType != "")
                {
                    sbInsert.Append(" N'" + syncRes.StructureType + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                }
                if (syncRes.ValidFrom != new DateTime())
                {
                    sbInsert.Append(" N'" + syncRes.ValidFrom.ToString(dateTimeformat) + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                }
                if (syncRes.ValidTo != new DateTime())
                {
                    sbInsert.Append(" N'" + syncRes.ValidTo.ToString(dateTimeformat) + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                ...
                if (syncRes.Result != -1)
                {
                    sbInsert.Append(" " + syncRes.Result + ",");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                ...
                if (syncRes.Remark != "")
                {
                    sbInsert.Append(" N'" + syncRes.Remark + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                }
                if (syncRes.CreatedBy != "")
                {
                    sbInsert.Append(" N'" + syncRes.CreatedBy + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
                }
                if (syncRes.CreatedTime != new DateTime())
                {
                    sbInsert.Append(" '" + syncRes.CreatedTime.ToString(dateTimeformat) + "',");
                }
                else
                {
                    sbInsert.Append("NULL, ");
