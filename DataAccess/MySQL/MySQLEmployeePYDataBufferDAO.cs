...
{
   public class MySQLEmployeePYDataBufferDAO:EmployeePYDataBufferDAO
    {
		...
		public MySQLEmployeePYDataBufferDAO()
		{
        ...
        public MySQLEmployeePYDataBufferDAO(MySqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
            dateTimeformat = dateTimeFormat.SortableDateTimePattern;
        ...
        public uint getMaxCalcID()
        {
            ...
            try
            {
                MySqlCommand cmd = new MySqlCommand("SELECT MAX(py_calc_id) AS calc_id FROM employee_py_data_buffers", conn);
