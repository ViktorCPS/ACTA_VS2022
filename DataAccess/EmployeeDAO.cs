...
namespace DataAccess
{
    ...
    public interface EmployeeDAO
    {
        ...
        List<EmployeeTO> ProlasciZaForKast(DateTime fromTime, DateTime toTime);

        List<EmployeeTO> ProlasciZaForKast(DateTime dateTime);

        int BrZaposlenih(DateTime mesec);
        int BrZaposlenih(DateTime fromTime, DateTime toTime);
        //  10.06.2019. BOJAN
        ...
        List<int> getEmplIDsForBankHours(DateTime mesec);
        List<EmployeeTO> listaRadnikaZaWU(int wuID);
        List<EmployeeTO> listaRadnikaZaWU(string name);
    }
}
