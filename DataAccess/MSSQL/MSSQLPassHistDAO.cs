...
{
    public class MSSQLPassHistDAO: PassHistDAO
    {
        SqlConnection conn = null;
        SqlTransaction _sqlTrans = null;
        protected string dateTimeformat = "";
        
        public MSSQLPassHistDAO()
		{
			conn = MSSQLDAOFactory.getConnection();
			DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
		}
        public MSSQLPassHistDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
            dateTimeformat = dateTimeFormat.SortableDateTimePattern;
        ...
        public List<PassHistTO> getPassHistInterval(PassHistTO phTO, DateTime fromTime, DateTime toTime, bool eventTimeChecked, string wUnits, DateTime fromModifiedTime, DateTime toModifiedTime, bool modifiedTimeChecked)
        {
            ...
            try
            {
                    ...
                    || (!phTO.ModifiedBy.Equals("")) || (((!fromModifiedTime.Equals(new DateTime())) || (!toModifiedTime.Equals(new DateTime()))) && modifiedTimeChecked))
                {
                    ...
                    {
                        //sb.Append(" UPPER(ph.employee_id) LIKE '" + employeeID.ToString().Trim().ToUpper() + "' AND");
                    ...
                    {
                        //sb.Append(" CONVERT(datetime,ph.event_time,101) >= '" + fromTime.ToString(dateTimeformat).Trim() + "' AND CONVERT(datetime,ph.event_time,101) <= '" + toTime.AddDays(1).ToString(dateTimeformat).Trim() + "' AND");
                        sb.Append(" ph.event_time >= CONVERT(datetime,'" + fromTime.ToString("yyyy-MM-dd") + "', 111) AND ");
                        sb.Append(" ph.event_time < CONVERT(datetime,'" + toTime.AddDays(1).ToString("yyyy-MM-dd") + "', 111) AND ");
                    }
                    if (((!fromModifiedTime.Equals(new DateTime())) || (!toModifiedTime.Equals(new DateTime()))) && modifiedTimeChecked)
                    {
                        //sb.Append("CONVERT(datetime, ph.event_time, 101) >= '" + fromTime.ToString(dateTimeformat).Trim() + "' AND CONVERT(datetime,ph.event_time,101) <= '" + toTime.AddDays(1).ToString(dateTimeformat).Trim() + "' AND ");
                        sb.Append("ph.modified_time >= convert(datetime,'" + fromModifiedTime.ToString("yyyy-MM-dd") + "', 111) AND ");
                        sb.Append("ph.modified_time < convert(datetime,'" + toModifiedTime.AddDays(1).ToString("yyyy-MM-dd") + "', 111) AND ");
                    }
                    if (phTO.PassTypeID != -1)
                    {
                        //sb.Append(" UPPER(ph.pass_type_id) LIKE '" + passTypeID.ToString().Trim().ToUpper() + "' AND");
                        sb.Append(" ph.pass_type_id = '" + phTO.PassTypeID.ToString().Trim() + "' AND");
                    }
                    if (phTO.LocationID != -1)
                    {
                        //sb.Append(" UPPER(ph.location_id) LIKE '" + locationID.ToString().Trim().ToUpper() + "' AND");
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        {
                            passHist.EventTime = (DateTime)row["event_time"];
                        }
                        ...
                        {
                            passHist.CreatedTime = (DateTime)row["created_time"];
                        }
                        ...
                        {
                            passHist.ModifiedTime = (DateTime)row["modified_time"];
                        }
        ...
        public int getPassesIntervalCount(PassHistTO phTO, DateTime fromTime, DateTime toTime, bool eventTimeChecked, string wUnits, DateTime fromModifiedTime, DateTime toModifiedTime, bool modifiedTimeChecked)
        {
            ...
            try
            {
                ...
                if ((phTO.EmployeeID != -1) || (!phTO.Direction.Trim().Equals("")) ||
                    (((!fromTime.Equals(new DateTime())) || (!toTime.Equals(new DateTime()))) && eventTimeChecked) ||
                    (phTO.PassTypeID != -1) || (phTO.LocationID != -1)
                    || (!phTO.ModifiedBy.Trim().Equals("")) || (((!fromModifiedTime.Equals(new DateTime())) || (!toModifiedTime.Equals(new DateTime()))) && modifiedTimeChecked))
                {
                    if (phTO.EmployeeID != -1)
                    {
                        //sb.Append("UPPER(ph.employee_id) LIKE '" + employeeID.ToString().Trim().ToUpper() + "' AND ");
