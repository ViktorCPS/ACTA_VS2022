...
{
    public class MSSQLRuleDAO : RuleDAO
    {
        ...
        public MSSQLRuleDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DAOController.GetInstance();
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
        ...
        public int searchReaderForRestaurant(string ruleType)
        {
            ...
            try
            {
                ...
                select = "SELECT rule_value FROM rules WHERE rule_type = '" + ruleType.Trim() + "'";
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    if (table.Rows[0]["rule_value"] != DBNull.Value)
                        id = Int32.Parse(table.Rows[0]["rule_value"].ToString());
                }
            }
            catch (Exception ex)
            {
