...
{
	/// <summary>
	/// Summary description for MSSQLGateTimeAccessProfileDAO.
	/// </summary>
	public class MSSQLGateTimeAccessProfileDAO : GateTimeAccessProfileDAO
	{
		...
		public MSSQLGateTimeAccessProfileDAO()
		{
			conn = MSSQLDAOFactory.getConnection();
		}

        public MSSQLGateTimeAccessProfileDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
        }
		...
		public GateTimeAccessProfileTO find(string gateTimeAccessProfileId)
		{
			...
			try
			{
				//SqlCommand cmd = new SqlCommand("SELECT * FROM gate_timeaccess_profiles_hdr WHERE gate_timeaccess_profile_id = " + Int32.Parse(gateTimeAccessProfileId.Trim()), conn );
                SqlCommand cmd;
                if (this.SqlTrans == null)
                    cmd = new SqlCommand("SELECT * FROM gate_timeaccess_profiles_hdr WHERE gate_timeaccess_profile_id = " + Int32.Parse(gateTimeAccessProfileId.Trim()), conn );
                else
                    cmd = new SqlCommand("SELECT * FROM gate_timeaccess_profiles_hdr WHERE gate_timeaccess_profile_id = " + Int32.Parse(gateTimeAccessProfileId.Trim()), conn, this.SqlTrans);
