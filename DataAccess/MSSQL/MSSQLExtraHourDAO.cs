...
{
	/// <summary>
	/// Summary description for MSSQLExtraHourDAO.
	/// </summary>
	public class MSSQLExtraHourDAO : ExtraHourDAO
	{
		...
		public MSSQLExtraHourDAO(SqlConnection connection)
		{
			conn = connection;
		}

        public MSSQLExtraHourDAO()
        {
			...
			try
			{
				...
				if (table.Rows.Count > 0)
				{
					foreach(DataRow row in table.Rows)
					{
						...
						{
							extraHourTO.EmployeeID = (int)row["employee_id"]; //Int32.Parse(row["employee_id"].ToString().Trim());							
						}					
						if (!row["date_earned"].Equals(DBNull.Value))
						{
							extraHourTO.DateEarned = (DateTime)row["date_earned"];
						}
						if (!row["extra_time_amt"].Equals(DBNull.Value))
						{
							extraHourTO.ExtraTimeAmt = (int)row["extra_time_amt"]; //Int32.Parse(row["extra_time_amt"].ToString().Trim());							
							extraHourTO.CalculatedTimeAmt = Util.Misc.transformMinToStringTime(extraHourTO.ExtraTimeAmt);
						}
			...
			try
			{
				...
				if (table.Rows.Count == 1)
				{
					foreach(DataRow row in table.Rows)
					{
						...
						if (!row["EmployeeSum"].Equals(DBNull.Value))
						{
							extraHourTO.EmployeeSum = (int)row["EmployeeSum"]; //Int32.Parse(row["EmployeeSum"].ToString().Trim());							
						}
						extraHourList.Add(extraHourTO);
					}
				}
