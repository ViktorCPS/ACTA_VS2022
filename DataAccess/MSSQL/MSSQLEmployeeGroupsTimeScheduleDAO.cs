...
{
    /// <summary>
    /// Summary description for MSSQLEmployeeGroupsTimeScheduleDAO.
    /// </summary>
    public class MSSQLEmployeeGroupsTimeScheduleDAO : EmployeeGroupsTimeScheduleDAO
    {
        ...
        public MSSQLEmployeeGroupsTimeScheduleDAO()
		{
			conn = MSSQLDAOFactory.getConnection();	
			DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
		}
        public MSSQLEmployeeGroupsTimeScheduleDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
			DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
            ...
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    foreach(DataRow row in table.Rows)
                    {
                        ...
                        if (!row["date"].Equals(DBNull.Value))
                        {
                            emplGroupsTimeScheduleTO.Date = (DateTime)row["date"];
                        }
                        if (!row.Equals(DBNull.Value))
                        {
            ...
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        if (!row["date"].Equals(DBNull.Value))
                        {
                            emplGroupTimeScheduleTO.Date = (DateTime)row["date"];
                        }
                        if (!row.Equals(DBNull.Value))
                        {
