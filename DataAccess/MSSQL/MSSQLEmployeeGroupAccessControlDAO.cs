...
{
	/// <summary>
	/// Summary description for MSSQLEmployeeGroupAccessControlDAO.
	/// </summary>
	public class MSSQLEmployeeGroupAccessControlDAO : EmployeeGroupAccessControlDAO
	{
		SqlConnection conn = null;

		public MSSQLEmployeeGroupAccessControlDAO()
		{
			conn = MSSQLDAOFactory.getConnection();
		}
        public MSSQLEmployeeGroupAccessControlDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
        }
		...
		public EmployeeGroupAccessControlTO find(string accessGroupId)
		{
			...
			try
			{
				SqlCommand cmd = new SqlCommand("SELECT access_group_id, name, description FROM employee_groups_access_control WHERE access_group_id = " + Int32.Parse(accessGroupId.Trim()), conn );
