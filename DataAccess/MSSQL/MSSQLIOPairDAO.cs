...
{
    /// <summary>
    /// Summary description for MSSQLIOPairsDAO.
    /// </summary>
    public class MSSQLIOPairDAO : IOPairDAO
    {
        SqlConnection conn = null;
        SqlTransaction _sqlTrans = null;
        protected string dateTimeformat = "";

        public MSSQLIOPairDAO()
        {
        ...
        public MSSQLIOPairDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
        ...
        //Tamara 6.2.2020. dodala gate_id
            ...
            DateTime startTime, DateTime endTime, int gateID, int manualyCreated, bool doCommit)
        {
            ...
            try
            {
                ...
                if (gateID != -1)
                {
                    sbInsert.Append(gateID.ToString().Trim() + ", ");
                }
                else
                {
                    sbInsert.Append("NULL, ");
        ...
        public int insert(IOPairTO pairTO, bool doCommit) 
        {
            ...
            try
            {
                ...
                if (pairTO.GateID != -1)
                {
                    sbInsert.Append(pairTO.GateID.ToString().Trim() + ", ");
                }
                else
                {
                    sbInsert.Append("NULL, ");
            ...
            catch (SqlException sqlex)
            {
                ...
                if (sqlex.Number == 2627)		// UNIQUE key constraint violation
                {
        ...
        //tamara 3.5.2018. ako je kontrola prolaska treba da postavi i parametar pair_processed_gen_used 
        public int insert2(IOPairTO pairTO, bool doCommit)
        {
            ...
            try
            {
                StringBuilder sbInsert = new StringBuilder();
                sbInsert.Append("INSERT INTO io_pairs ");
                sbInsert.Append("(io_pair_date,pair_processed_gen_used, employee_id, location_id, is_wrk_hrs_counter, pass_type_id, start_time, end_time, gate_id, manual_created, created_by, created_time) ");
                sbInsert.Append("VALUES (");
            ...
            int passTypeID, DateTime startTime, DateTime endTime, int manualyCreated, bool doCommit)
        {
            ...
            try
            {
                ...
                sbInsert.Append(" N'" + Constants.extraHoursCreatedBy + "', GETDATE()) ");
        ...
        private bool delete(int employeeID, string date, bool doCommit)
        {
            ...
            try
            {
                ...
                sbDelete.Append("AND convert(varchar(20), io_pair_date, 101) = '" + date.Trim() + "' ");
        ...
        public IOPairTO find(int IOPairID)
        {
            ...
            try
            {
                ...
                if (table.Rows.Count == 1)
                {
                    ...
                    {
                        pairTO.IOPairDate = (DateTime)table.Rows[0]["io_pair_date"];
                    }
                    ...
                    {
                        pairTO.StartTime = (DateTime)table.Rows[0]["start_time"];
                    }
                    if (table.Rows[0]["end_time"] != DBNull.Value)
                    {
                        pairTO.EndTime = (DateTime)table.Rows[0]["end_time"];
                    }
        ...
        public DateTime getFirstArrivedTime(int emplID, DateTime date)
        {
            ...
            try
            {
                ...
                sb.Append(" WHERE start_time IS NOT NULL AND end_time IS NOT NULL AND employee_id = " + emplID.ToString() + " AND  CONVERT(VARCHAR(24), io_pair_date, 120) = '" + date.ToString("yyy-MM-dd HH:mm:ss") + "'");
                ...
                if (table.Rows.Count > 0)
                {
                    ...
                    if (row["first_arrived"] != DBNull.Value)
                        arrived = (DateTime)row["first_arrived"];
                }
            }
            catch (Exception ex)
            {
        ...
        public List<IOPairTO> getIOPairsClosed(IOPairTO pairTO)
        {
            ...
            try
            {
                    ...
                    (pairTO.ManualCreated != -1) || (pairTO.ProcessedGenUsed != -1))
                {
                    ...
                    if (pairTO.ProcessedGenUsed != -1)
                    {
                        sb.Append(" pair_processed_gen_used = '" + pairTO.ProcessedGenUsed.ToString().Trim() + "' AND");
                    }
                    if (!pairTO.IOPairDate.Equals(new DateTime()))
                    {
                    ...
                    // tamara 17.7.2018.
                    // Nenad 11. VIII 2017. Process all pass types, not just regular work (pass_type_id = 0)
                    //if (pairTO.PassTypeID != -1)
                    //{
                    ...
                    if (!pairTO.StartTime.Equals(new DateTime()))
                    {
                        sb.Append(" CONVERT(VARCHAR(24), start_time, 120) = '" + pairTO.StartTime.ToString("yyy-MM-dd HH:mm:ss") + "' AND");
                    }
                    if (!pairTO.EndTime.Equals(new DateTime()))
                    {
                        sb.Append(" CONVERT(VARCHAR(24), end_time, 120) = '" + pairTO.EndTime.ToString("yyy-MM-dd HH:mm:ss") + "' AND");
                    }
                    if (pairTO.ManualCreated != -1)
                    {
                ...
                SqlCommand cmd = new SqlCommand(select + " ORDER BY start_time", conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<IOPairTO> getIOPairsClosed(IOPairTO pairTO, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                    ...
                    (pairTO.ManualCreated != -1) || (pairTO.ProcessedGenUsed != -1) || (fromDate != new DateTime()) || (toDate != new DateTime()))
                {
                    ...
                    {
                        sb.Append(" CONVERT(VARCHAR(24), io_pair_date, 120) >= '" + fromDate.ToString("yyy-MM-dd HH:mm:ss") + "' AND");
                    }
                    if (!toDate.Equals(new DateTime()))
                    {
                        sb.Append(" CONVERT(VARCHAR(24), io_pair_date, 120) <= '" + toDate.ToString("yyy-MM-dd HH:mm:ss") + "' AND");
                    }
                    if (pairTO.EmployeeID != -1)
                    {
                        sb.Append(" employee_id = '" + pairTO.EmployeeID.ToString().Trim() + "' AND");
        ...
        public int insert(IOPairTO ioPairTO, PassTO firstPass, PassTO secondPass, DAOFactory factory)
        {
            ...
            try
            {
                ...
                {
                    //ako je kontrola prolaska postavljen je parametar processedGenUsed na 1 i f-ja insert2 se poziva da bi 
                    //postavila tu vrednost jer nema potrebe da se dalje procesira u io_pair_processed tabeli
        ...
        public int getIOPairsForEmplDateCount(DateTime fromDate, DateTime toDate, List<int> employeesList)
        {
            ...
            try
            {
                ...
                {
                    StringBuilder sb = new StringBuilder();
                    sb.Append("SELECT count(iop.io_pair_id) count ");
                    sb.Append("FROM io_pairs iop, employees empl, locations loc, pass_types pt,working_units wu WHERE");
        ...
        public List<IOPairTO> getIOPairsAllEmplDatePairs(string employeeIDString, List<DateTime> datesList)
        {
            ...
            try
            {
                ...
                if (datesList.Count > 0)
                {
                    ...
                    {
                        sb.Append("convert(datetime,'" + currentDate.ToString("yyyy-MM-dd") + "', 111), ");
                    }
        ...
        public List<IOPairTO> getIOPairsEmplTimeInterval(string employeeIDString, DateTime fromDate, DateTime toDate, IDbTransaction trans)
        {
            ...
            try
            {
                ...
                if (!fromDate.Equals(new DateTime()))
                {
                    sb.Append(" AND io_pair_date >= convert(datetime,'" + fromDate.ToString("yyyy-MM-dd") + "', 111)");
                }
                if (!toDate.Equals(new DateTime()))
                {
                    sb.Append(" AND io_pair_date < convert(datetime,'" + toDate.AddDays(1).ToString("yyyy-MM-dd") + "', 111)");
                }
        ...
        public List<IOPairTO> getIOPairsForLoc(int locationID, string locations, string wUnits, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand(sb.ToString() + "ORDER BY loc.name, empl.last_name, empl.first_name, iop.io_pair_date ", conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public DataSet getIOPairsForLocDataSet(int locationID, string locations, string wUnits, DateTime fromDate, DateTime toDate)
        {
            ...
            try
            {
                ...
                //SqlCommand cmd = new SqlCommand(sb.ToString() + "ORDER BY iop.location_id, empl.last_name ", conn);
                SqlCommand cmd = new SqlCommand(sb.ToString(), conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<IOPairTO> getIOPairs(IOPairTO pairTO, DateTime fromDate, DateTime toDate, string wUnits, int workingUnitID)
        {
            ...
            try
            {
                ...
                if (!fromDate.Equals(new DateTime()))
                {
                    //sb.Append("CONVERT(datetime,iop.io_pair_date,101) >= '" + fromDate.ToString(dateTimeformat) + "' AND ");
                    sb.Append("iop.io_pair_date >= convert(datetime,'" + fromDate.ToString("yyyy-MM-dd") + "', 111) AND ");
                }

                if (!toDate.Equals(new DateTime()))
                {
                    //sb.Append("CONVERT(datetime,iop.io_pair_date,101) < '" + toDate.AddDays(1).ToString(dateTimeformat) + "' AND ");
                    sb.Append("iop.io_pair_date < convert(datetime,'" + toDate.AddDays(1).ToString("yyyy-MM-dd") + "', 111) AND ");
                }
                ...
                SqlCommand cmd = new SqlCommand(sb.ToString() + "ORDER BY empl.last_name ", conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<IOPairTO> getIOPairs(IOPairTO pairTO, DateTime fromDate, DateTime toDate, string wUnits, int workingUnitID, IDbTransaction trans)
        {
            ...
            try
            {
                ...
                SqlCommand cmd = new SqlCommand(sb.ToString() + "ORDER BY empl.last_name ", conn, (SqlTransaction)trans);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public int getIOPairsCount(IOPairTO pairTO, DateTime fromDate, DateTime toDate, string wUnits, int workingUnitID)
        {
            ...
            try
            {
                ...
                if (pairTO.PassTypeID != -1)
                {
                    sb.Append("iop.pass_type_id = " + pairTO.PassTypeID + " AND ");
                }
        ...
        public List<IOPairTO> getIOPairsForEmpl(DateTime fromDate, DateTime toDate, List<int> employeesList, int locationID)
        {
            ...
            try
            {
                if (employeesList.Count > 0)
                {
                    ...
                    select = sb.ToString().Substring(0, sb.ToString().Length - 3);
                    SqlCommand cmd = new SqlCommand(select + "ORDER BY iop.employee_id, iop.io_pair_date, iop.start_time, loc.name ", conn);
                    SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<IOPairTO> getIOPairsForEmployees(DateTime fromDate, DateTime toDate, List<int> employeesList, int locationID)
        {
            ...
            try
            {
                if (employeesList.Count > 0)
                {
                    ...
                    select = sb.ToString().Substring(0, sb.ToString().Length - 3);
                    SqlCommand cmd = new SqlCommand(select + "ORDER BY iop.employee_id, iop.io_pair_date, loc.name ", conn);
                    SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<DateTime> getDatesWithOpenPairs(DateTime fromDate, DateTime toDate, int employeeID)
        {
            ...
            try
            {
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        datesList.Add((DateTime)row["io_pair_date"]);
                    }
                }
            }
            catch (Exception ex)
        ...
        public List<IOPairTO> getIOPairsForPresence(int wuID, string wUnitsString, DateTime from, DateTime to) 
        {
            ...
            try
            {
                ...
                sb.Append("FROM io_pairs_processed ip, employees empl, working_units wu, pass_types pt, locations loc ");
                sb.Append("WHERE ");
        ...
        public List<IOPairTO> getIOPairsForOUPresence(int ouID, string oUnitsString, DateTime from, DateTime to) // Nenad 09. X 2017. Search by organizational units
        {
            DataSet dataSet = new DataSet();
            IOPairTO pairTO = new IOPairTO();
            List<IOPairTO> pairsList = new List<IOPairTO>();
            ...
            try
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT ip.*, empl.first_name first_name, empl.last_name last_name, wu.name wu_name, pt.description pass_type_desc ,loc.name loc_name ");
                sb.Append("FROM io_pairs_processed ip, employees empl, organizational_units wu, pass_types pt, locations loc ");
                sb.Append("WHERE ");
                sb.Append("ip.employee_id = empl.employee_id AND ");
                sb.Append("empl.organizational_unit_id = wu.organizational_unit_id AND ");
                sb.Append("ip.pass_type_id = pt.pass_type_id AND ");
                sb.Append("ip.location_id = loc.location_id AND ");

                if (ouID != -1)
                {
                    sb.Append(" empl.organizational_unit_id = '" + ouID.ToString().Trim() + "' AND ");
                }
                else
                {
                    sb.Append(" empl.organizational_unit_id IN (" + oUnitsString + ") AND ");
                }
        ...
        /// <param name="to">End date</param>
        /// <returns>ArrayList of EmployeeTO objects</returns>
        public List<EmployeeTO> getDistinctOUEmployees(DateTime from, DateTime to, int organizationalUnitID, string oUnits) // Nenad 09. X 2017. Search by organizational units
        {
            List<EmployeeTO> employeesTOList = new List<EmployeeTO>();
            DataSet dataset = new DataSet();
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT DISTINCT ip.employee_id employee_id, empl.first_name first_name, empl.last_name last_name, empl.organizational_unit_id organizational_unit, empl.status empl_status, ou.name ou_name  ");
                sb.Append("FROM io_pairs ip, employees empl, organizational_units ou ");
                sb.Append("WHERE empl.employee_id = ip.employee_id AND ");
                sb.Append("ou.organizational_unit_id = empl.organizational_unit_id AND ");
                ...
                {
                    sb.Append("empl.organizational_unit_id = " + organizationalUnitID + " AND ");
                }
                else
                {
                    sb.Append("empl.organizational_unit_id IN (" + oUnits + ") AND ");
                }

                sb.Append("ip.start_time IS NOT NULL AND ");
                sb.Append("ip.end_time IS NOT NULL ");
                sb.Append("GROUP BY ip.employee_id, empl.first_name, empl.last_name, empl.organizational_unit_id, ou.name, empl.status ");
                sb.Append("ORDER BY  empl.last_name, empl.first_name ");
                ...
                foreach (DataRow row in dataTable.Rows)
                {
                    ...
                    employeeTO.FirstName = row["first_name"].ToString();
                    employeeTO.LastName = row["last_name"].ToString();
                    employeeTO.OrgUnitID = (int)row["organizational_unit"];
                    employeeTO.WorkingUnitName = row["ou_name"].ToString();
                    employeeTO.Status = row["empl_status"].ToString();
        ...
        public ArrayList getEmpoloyeesByDate(DateTime fromDate)
        {
            ...
            try
            {
                ...
                sb.Append("WHERE CONVERT(datetime, io_pair_date, 101) <= '" + DateTime.Now.ToString("MM/dd/yyy") + "' ");
                if (!fromDate.Equals(new DateTime()))
                    sb.Append("AND CONVERT(datetime, io_pair_date, 101) >= '" + fromDate.ToString("MM/dd/yyy") + "' ");
                ...
                foreach (DataRow row in table.Rows)
                {
                    ...
                    member.Insert(0, (DateTime)row["io_pair_date"]);
        ...
        public List<IOPairTO> getOpenPairs(int employeeID, DateTime date)
        {
            ...
            try
            {
                ...
                sb.Append("AND CONVERT(datetime, io_pair_date, 101) = '" + date.ToString("MM/dd/yyy") + "' ");
                sb.Append("AND (start_time IS NULL OR end_time IS NULL)");
        ...
        public List<IOPairTO> getPairsWithSpecialOut(DateTime start, DateTime end, DateTime date)
        {
            ...
            try
            {
                ...
                sb.Append("WHERE start_time IS NOT NULL AND end_time IS NOT NULL ");
                sb.Append("AND CONVERT(datetime, io_pair_date, 101) = '" + date.ToString("MM/dd/yyy") + "' ");
                sb.Append("AND ((CONVERT(varchar(20), start_time, 108) >= '" + start.ToString("HH:mm:ss") + "' ");
                sb.Append("AND CONVERT(varchar(20), start_time, 108) <= '" + end.ToString("HH:mm:ss") + "') ");
                sb.Append("OR (CONVERT(varchar(20), end_time, 108) >= '" + start.ToString("HH:mm:ss") + "' ");
                sb.Append("AND CONVERT(varchar(20), end_time, 108) <= '" + end.ToString("HH:mm:ss") + "')) ");
                sb.Append("AND DATEADD(s, 1, start_time) < end_time");
        ...
        public List<IOPairTO> getPermissionPassPairs()
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT * FROM io_pairs WHERE end_time = DATEADD(s, 1, start_time)");
        ...
        public bool existEmlpoyeeDatePair(int employeeID, DateTime ioPairDate, DateTime startTime, DateTime endTime)
        {
            ...
            try
            {
                ...
                if (employeeID != -1)
                {
                    ...
                    {
                        //varchar(5), just HH:mm
                        sb.Append(" AND (");

                        sb.Append(" (");
                        sb.Append(" CONVERT(VARCHAR(5), start_time, 108) <= '" + startTime.ToString("HH:mm") + "'");
                        sb.Append(" AND CONVERT(VARCHAR(5), end_time, 108) > '" + startTime.ToString("HH:mm") + "'");
                        sb.Append(" )");

                        sb.Append(" OR (");
                        sb.Append(" CONVERT(VARCHAR(5), start_time, 108) > '" + startTime.ToString("HH:mm") + "'");
                        sb.Append(" AND CONVERT(VARCHAR(5), start_time, 108) < '" + endTime.ToString("HH:mm") + "'");
                        sb.Append(" )");
                    ...
                    {
                        //varchar(5), just HH:mm
                        sb.Append(" AND CONVERT(VARCHAR(5), start_time, 108) = '" + startTime.ToString("HH:mm") + "'");
                    }
                    if (!endTime.Equals(new DateTime()))
                    {
                        //varchar(5), just HH:mm
                        sb.Append(" AND CONVERT(VARCHAR(5), end_time, 108) = '" + endTime.ToString("HH:mm") + "'");
                    }*/
        ...
        public DataSet getOpenPairs()
        {
            ...
            try
            {
                string select = "SELECT io_pair_id, CONVERT(CHAR(10), io_pair_date, 101) AS io_pair_date, employee_id, location_id, is_wrk_hrs_counter, pass_type_id, start_time, end_time, manual_created, created_by, created_time, modified_by, modified_time FROM io_pairs WHERE start_time IS NULL OR end_time IS NULL";
                SqlCommand cmd = new SqlCommand(select, conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public ArrayList getEmployeesByDateAutoClose()
        {
            ...
            try
            {
                sb.Append("SELECT DISTINCT employee_id, CONVERT (varchar (20), io_pair_date, 101) AS workday");
                sb.Append(" FROM io_pairs");
                sb.Append(" WHERE");
        ...
        public List<IOPairTO> getIOPairsForEmplPerm(DateTime fromDate, DateTime toDate, List<int> employeesList)
        {
            ...
            try
            {
                if (employeesList.Count > 0)
                {
                    ...
                    SqlCommand cmd = new SqlCommand(select + "ORDER BY iop.employee_id, iop.io_pair_date, iop.start_time ", conn);
                    SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public List<IOPairTO> getNonEnteredIOPairs(int employeeID, DateTime startTime, DateTime endTime)
        {
            ...
            try
            {
                ...
                sb.Append("AND iop.io_pair_date >= '" + startTime.ToString("yyyy-MM-dd") + "' AND");
                sb.Append(" iop.io_pair_date <= '" + endTime.AddDays(1).ToString("yyyy-MM-dd") + "' AND");
                sb.Append(" iop.employee_id = empl.employee_id ");
                sb.Append("AND empl.employee_id =" + employeeID);
        ...
        //15.06.2017 Miodrag Mitrovic funkcija koja ne odseca vreme i vraca samo parove za end_time=null i sortira po organizacionim jedinicama
        ...
        public List<IOPairTO> getIOPairsEmplTimeInterval2(string employeeIDString, DateTime fromDate, DateTime toDate, IDbTransaction trans)
        {
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT iop.io_pair_id, iop.io_pair_date, iop.employee_id, iop.start_time, iop.end_time, iop.pass_type_id");
                sb.Append(" FROM io_pairs as iop, organizational_units as ou, employees as e ");
                sb.Append(" WHERE iop.employee_id=e.employee_id AND e.[organizational_unit_id]=ou.[organizational_unit_id] ");

                /*

                if (employeeIDString != "")
                {
                    sb.Append("  employee_id IN (" + employeeIDString + ")");
                }
                */

                if (!fromDate.Equals(new DateTime()))
                {
                    sb.Append(" AND start_time >= '" + fromDate.ToString("yyyy-MM-dd HH:mm:ss.000") + "'");
                    //sb.Append(" AND start_time >= '" + fromDate.ToString("2019-06-17 08:00:58.000") + "'");
                }
                if (!toDate.Equals(new DateTime()))
                {
                    sb.Append(" AND start_time <= '" + toDate.ToString("yyyy-MM-dd HH:mm:ss.000") + "'");
                }

                sb.Append(" AND end_time IS NULL ORDER BY ou.[organizational_unit_id], iop.start_time, iop.employee_id, iop.io_pair_date");
                select = sb.ToString();
        ...
        public List<IOPairTO> getOpenPairsForReport(string employeeID, DateTime from, DateTime to, string orgUnitID, string workingUnitID) {
            ...
            try {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT ip.io_pair_date, l.name as 'location', tp.description, e.last_name+'  '+e.first_name as 'full_name', ip.start_time, ip.end_time ");
                sb.Append(" FROM ((io_pairs_processed ip INNER JOIN locations l on l.location_id = ip.location_id) INNER JOIN employees e on e.employee_id = ip.employee_id) INNER JOIN pass_types tp on tp.pass_type_id = ip.pass_type_id ");
                sb.Append(" WHERE ");
                if (!employeeID.Equals("-1")) {
                    sb.Append(" e.employee_id in (" + employeeID.Trim() + ") AND ");
                } else 
                {
                    sb.Append(" e.employee_id = -1 AND ");
                }

                if (!orgUnitID.Equals("-1")) {
                    sb.Append(" e.organizational_unit_id in (" + orgUnitID.Trim() + ") AND ");
                }
                else {
                    sb.Append(" e.organizational_unit_id = -1 AND ");
                }

                if (!workingUnitID.Equals("-1")) {
                    sb.Append(" e.working_unit_id in (" + workingUnitID.Trim() + ") AND ");
                }
                else {
                    sb.Append(" e.working_unit_id = -1 AND ");
                }

                sb.Append(" ip.io_pair_date BETWEEN '"+from.ToString()+"' AND '" + to.ToString() + "' "); //date.ToString("MM/dd/yyy")
                sb.Append(" AND ip.pass_type_id in (-100,-1000,59,88) ");
                //sb.Append(" AND (ip.start_time IS NULL OR ip.end_time IS NULL) AND ip.pass_type_id in (-100,-1000,59,88) ");
                ...
                if (table.Rows.Count > 0) {
                    foreach (DataRow row in table.Rows) {
                        ...
                        if (row["location"] != DBNull.Value) {
                            pairTO.LocationName = row["location"].ToString().Trim();
                        }
                        if (row["description"] != DBNull.Value) {
                            pairTO.PassType= row["description"].ToString().Trim();
                        }
                        if (row["full_name"] != DBNull.Value) {
                            pairTO.EmployeeLastName = row["full_name"].ToString().Trim();
                        }
                        if (row["start_time"] != DBNull.Value) {
                            pairTO.StartTime = (DateTime)row["start_time"];
                        }
                        else {
                            pairTO.StartTime = new DateTime();
                        }
                        if (row["end_time"] != DBNull.Value) {
                            pairTO.EndTime = (DateTime)row["end_time"];
                        }
                        else {
                            pairTO.EndTime = new DateTime();
                        }
                        //if (row["manual_created"] != DBNull.Value) {
                        //    pairTO.ManualCreated = Int32.Parse(row["manual_created"].ToString().Trim());
                        //}
        ...
        //  23.01.2020. BOJAN
        public List<IOPairTO> getIOPairsForBreaks(DateTime from, DateTime to, string emplIDs, string gateIDs)
        {
            DataSet dataSet = new DataSet();
            IOPairTO pair = new IOPairTO();
            List<IOPairTO> pairsList = new List<IOPairTO>();
            ...
            {
                StringBuilder sb = new StringBuilder();
                sb.Append("SELECT e.employee_id, e.first_name, e.last_name, iop.io_pair_date, g.name, iop.start_time, iop.end_time, CONVERT(varchar(8), DATEADD(second, DATEDIFF(second, iop.start_time, iop.end_time), 0), 114) AS 'time', g.gate_id, g.gate_type ");
                sb.Append("FROM (io_pairs iop INNER JOIN employees e ON e.employee_id = iop.employee_id) INNER JOIN gates g ON g.gate_id = iop.gate_id ");
                sb.Append("WHERE ");
                sb.Append("iop.io_pair_date BETWEEN '" + from + "' AND '" + to + "' AND ");
                sb.Append("e.employee_id IN (" + emplIDs + ") AND ");
                sb.Append("g.gate_id IN (" + gateIDs + ") ");

                SqlCommand cmd = new SqlCommand(sb.ToString() + "ORDER BY iop.io_pair_date ", conn);
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
                ...
                if (table.Rows.Count > 0)
                {
                    foreach (DataRow row in table.Rows)
                    {
                        ...
                        if (row["first_name"] != DBNull.Value)
                        {
                            pair.EmployeeName = row["first_name"].ToString().Trim();
                        }
                        if (row["last_name"] != DBNull.Value)
                        {
                            pair.EmployeeLastName = row["last_name"].ToString().Trim();
                        }
                        if (row["io_pair_date"] != DBNull.Value)
                        {
                            pair.IOPairDate = (DateTime)row["io_pair_date"];
                        ...
                        {
                            pair.PassType = row["name"].ToString().Trim();
                        }
                        if (row["time"] != DBNull.Value)
                        {
                            pair.Time = row["time"].ToString().Trim();
                        }
        ...
        public void deleteForDay(int emplID, DateTime date)
        {
            try
            {
                string delete = "DELETE FROM actamgr.io_pairs WHERE employee_id="+emplID+" and io_pair_date='"+date.ToString("yyyy-MM-dd")+"'";
                SqlCommand cmdDel = new SqlCommand(delete, conn);
                cmdDel.CommandTimeout = 1200;
        ...
        public void deleteForAutoTimeSchema(int emplID, DateTime date, bool doCommit)
        {
            ...
            try
            {
                string delete = "DELETE FROM actamgr.io_pairs WHERE employee_id=" + emplID.ToString() + " and io_pair_date='" + date.Date.ToString("yyyy-MM-dd") + "'";
