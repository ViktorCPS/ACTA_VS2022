...
{
	/// <summary>
	/// Summary description for MSSQLEmployeeAbsenceDAO.
	/// </summary>
	public class MSSQLEmployeeAbsenceDAO : EmployeeAbsenceDAO
	{
		...
		public MSSQLEmployeeAbsenceDAO()
		{
			conn = MSSQLDAOFactory.getConnection();
			DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
		}
        public MSSQLEmployeeAbsenceDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
              DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
			dateTimeformat = dateTimeFormat.SortableDateTimePattern;
		...
		{
			MSSQLIOPairDAO iopDAO = (MSSQLIOPairDAO) factory.getIOPairDAO(conn);
            ...
            if (trans != null)
            {
                this.SqlTrans = (SqlTransaction) trans;
            }
            else
            {
        ...
        {
            MSSQLIOPairDAO iopDAO = (MSSQLIOPairDAO)factory.getIOPairDAO(null);
			...
			DateTime dateEnd, DAOFactory factory)
		{
			...
			try
			{
				...
				SqlCommand cmd = new SqlCommand( sbDelete.ToString(), conn, this.SqlTrans);
				int res = cmd.ExecuteNonQuery();
				if (res != 0)
				{
					isDeleted = true;
		...
		public EmployeeAbsenceTO find(int recID)
		{
			...
			try
			{
				SqlCommand cmd = new SqlCommand( "SELECT * FROM employee_absences WHERE rec_id = '" + recID.ToString().Trim() + "'", conn );
				...
				if (table.Rows.Count == 1)
				{
					...
					{
						emplAbsence.DateStart = (DateTime) table.Rows[0]["date_start"];
					}
					if (table.Rows[0]["date_end"] != DBNull.Value)
					{
						emplAbsence.DateEnd = (DateTime) table.Rows[0]["date_end"];
					}
			...
			try
			{
				...
				if (table.Rows.Count > 0)
				{
					foreach(DataRow row in table.Rows)
					{
						...
						{
							emplAbsence.DateStart = (DateTime) row["date_start"];
						}
                        if (row["created_time"] != DBNull.Value)
                        {
                            emplAbsence.CreatedTime = (DateTime)row["created_time"];
                        }
						if (row["date_end"] != DBNull.Value)
						{
							emplAbsence.DateEnd = (DateTime) row["date_end"];
						}
			...
			try
			{
				...
				if (table.Rows.Count > 0)
				{
					foreach(DataRow row in table.Rows)
					{
                        ...
                        {
                            emplAbsence.VacationYear = (DateTime)row["vac_year"];
                        }
