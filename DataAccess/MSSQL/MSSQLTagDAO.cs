...
{
	/// <summary>
	/// Summary description for MSSQLTagDAO.
	/// </summary>
	public class MSSQLTagDAO : TagDAO
	{
        ...
        public MSSQLTagDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
            dateTimeformat = dateTimeFormat.SortableDateTimePattern;
        ...
        public int insert(ulong tagID, int ownerID, string status, string description, DateTime issued, DateTime validTO, string createdBy)
        {
            ...
            try
            {
                ...
                sbInsert.Append("N'" + createdBy.Trim() + "', GETDATE()) ");
		...
		public TagTO find(int recordID)
		{
			...
			try
			{
				SqlCommand cmd = new SqlCommand( "SELECT * FROM tags WHERE record_id = '" + recordID.ToString().Trim() + "'", conn );
		...
		public TagTO findActive(int ownerID)
		{
			...
			try
			{
				...
				//	+ "' OR status = '" + Constants.statusBlocked + "')";
            ...
            {                
                    ...
                    (!tagID.Equals("")))
                {
                    ...
                    if (!from.Equals(new DateTime(0)) && !to.Equals(new DateTime(0)))
                    {
                        ...
                        sb.Append("((t.created_time >= convert(datetime,'" + from.ToString("yyyy-MM-dd") + "', 111) AND ");
                        sb.Append("t.created_time < convert(datetime,'" + to.AddDays(1).ToString("yyyy-MM-dd") + "', 111)) OR ");
                        sb.Append("(t.modified_time >= convert(datetime,'" + from.ToString("yyyy-MM-dd") + "', 111) AND ");
                        sb.Append("t.modified_time < convert(datetime,'" + to.AddDays(1).ToString("yyyy-MM-dd") + "', 111))) AND ");
                    }
                    if (!tagID.Equals(""))
                    {
		...
		public List<TagTO> getTags(TagTO t)
		{
			...
			try
			{
				...
				SqlCommand cmd = new SqlCommand(select + " ORDER BY record_id DESC", conn );
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
		...
		public Dictionary<ulong, TagTO> getActiveTags()
		{
			...
			{
				StringBuilder sb = new StringBuilder();
                sb.Append("SELECT t.*,e.access_group_id as access_group_id FROM tags t, employees e WHERE t.owner_id=e.employee_id AND");

				sb.Append(" UPPER(t.status) LIKE N'" + Constants.statusActive.ToUpper() + "' ");
		...
		public Dictionary<ulong, TagTO> getActiveTagsWithAccessGroup()
		{
			...
			try
			{
                ...
                if (this.SqlTrans == null)
                    ...
                    cmd = new SqlCommand(sb.ToString(), conn, this.SqlTrans);
				SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
		...
		public List<TagTO> getInactiveTags(string wUnits, DateTime from, DateTime to)
		{
			...
			try
			{
				...
				sb.Append("AND CONVERT(DATETIME, t.modified_time, 101) >= '" + from.ToString("MM/dd/yyy") + "' ");
				sb.Append("AND CONVERT(DATETIME, t.modified_time, 101) <= '" + to.ToString("MM/dd/yyy") + "' ");
				...
				if (table.Rows.Count > 0)
				{
					foreach(DataRow row in table.Rows)
					{
						...
						if (row["modified_time"] != DBNull.Value)
						{
							tag.ModifiedTime = (DateTime) row["modified_time"];
						}
						if (row["owner_id"] != DBNull.Value)
						{
