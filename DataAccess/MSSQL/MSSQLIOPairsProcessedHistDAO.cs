...
{
   public class MSSQLIOPairsProcessedHistDAO:IOPairsProcessedHistDAO
    {
        ...
        public MSSQLIOPairsProcessedHistDAO()
        {
        ...
        public MSSQLIOPairsProcessedHistDAO(SqlConnection sqlConnection)
        {
            conn = sqlConnection;
            DAOController.GetInstance();
            DateTimeFormatInfo dateTimeFormat = new CultureInfo("en-US", true).DateTimeFormat;
        ...
        public IOPairsProcessedHistTO find(uint recID)
        {
            ...
            try
            {
                SqlCommand cmd = new SqlCommand("SELECT * FROM io_pairs_processed_hist WHERE rec_id = '" + recID.ToString().Trim() + "'", conn);
                cmd.CommandTimeout = Constants.commandTimeout;
                SqlDataAdapter sqlDataAdapter = new SqlDataAdapter(cmd);
        ...
        public bool isAlert(int emplID, DateTime date)
        {
            ...
            try
            {
                string select = "SELECT COUNT(*) num FROM io_pairs_processed_hist WHERE employee_id = '" + emplID.ToString().Trim() + "' AND io_pair_date = '" 
                    + date.Date.ToString(dateTimeformat) + "' AND alert = '" + Constants.alertStatus + "'";
